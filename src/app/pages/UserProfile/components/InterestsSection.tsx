          ...
          interests.map((interest, index) => (
            <div key={`${interest}-${index}`}>
              <div className="flex justify-between items-center">
                {editingIndex === index ? (
                  <input
                    className="flex-1 lg:mr-3 lg:rounded-md border border-stroke-grey bg-primary-lm focus:outline-accent-lm lg:text-md lg:px-3 lg:py-1"
                    autoFocus
                    value={editingValue}
              ...
              {index !== interests.length - 1 && <hr className="lg:mt-2 border-stroke-grey" />}
            </div>
          ))
        ) : (
          <p className="text-text-lighter-lm">No interests added yet.</p>
        )}
      </div>
    </div>
  );
