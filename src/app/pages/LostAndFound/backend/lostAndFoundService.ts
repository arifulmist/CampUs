...
export async function createLostAndFoundPost({
  ...
  try {
    ...
    const [_created] = await fetchLostAndFoundPosts({
      limit: 1,
      order: "newest",
    });
    ...
    const deptLookup = userInfoData?.departments_lookup;
    const deptName =
      (Array.isArray(deptLookup)
        ? deptLookup[0]?.department_name
        : (deptLookup as any)?.department_name) ??
...
export async function fetchLostAndFoundPostById(
  ...
  try {
    ...
    const authorName = userInfoData?.name ?? "Unknown";
    const deptLookup2 = userInfoData?.departments_lookup;
    const deptName =
      (Array.isArray(deptLookup2)
        ? deptLookup2[0]?.department_name
        : (deptLookup2 as any)?.department_name) ??
      (typeof userInfoData?.department === "string"
        ? userInfoData?.department
        : undefined);
    const batch = userInfoData?.batch ?? undefined;
